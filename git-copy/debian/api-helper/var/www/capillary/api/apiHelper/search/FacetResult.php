...
class FacetResult{
   ...
   private function parseData()   
   {
      ...
      $this->logger->debug("@@solr: " . $this->raw_data);
      $facet_count_by_fields = $data['facet_counts']['facet_fields'];
      $result_doc = array();
      foreach($facet_count_by_fields as $k => $v)
          $key = $k;
          foreach($v as $i => $j)
              $result_doc[] = array("key" => $key, "value" => $i, "count" => $j);
      $this->setCount(count($result_doc));      
      $this->documents = $result_doc;
   }
